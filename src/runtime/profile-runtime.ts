...
export async function resolveProfileRuntime(
  ...
  if (!profile && opts.allowBootstrap) {
    ...
    if (detected.length === 0) {
      throw new Error('未找到 ChatGPT/Codex Desktop，请先安装并登录桌面应用');
    }
  ...
  if (isComplete(existing)) {
    ...
    const profileConfig = createRuntimeProfileConfig({
      agentKind: requestedAgent ?? 'codex',
  ...
  const bootstrapAgent = resolveBootstrapAgent(requestedAgent, profile) ?? 'codex';
...
function displayAgentKind(kind: AgentKind): string {
  return kind === 'claude' ? 'Claude Code' : 'Codex Desktop';
}
...
async function maybeMigratePlaintextSecret(
  ...
  if (typeof s === 'string' && !/^\$\{[A-Z][A-Z0-9_]*\}$/.test(s)) {
    try {
      ...
      console.log('🔒 已把 App Secret 加密迁移到 ~/.codex-lark/profiles/<profile>/secrets.enc');
      return next;
    } catch (err) {
