...
export async function runRegistrationWizard(
  ...
  const tenant = config.accounts.app.tenant;
  const operatorOpenId = config.operatorOpenId;

  console.log('\n✓ 应用创建成功');
  console.log(`  App ID:  ${config.accounts.app.id}`);
...
/**
 * Create a PersonalAgent application with the official Lark channel flow.
 * UI frontends receive progress through callbacks; the App Secret stays in
 * this process and is never sent to the browser dashboard.
 */
export async function registerCodexLarkApp(
  progress: RegistrationProgress = {},
): Promise<AppConfig & { operatorOpenId?: string }> {
  const result = await registerApp({
    source: 'codex-lark',
    onQRCodeReady: (info) => progress.onQRCodeReady?.(info),
    onStatusChange: (info) => progress.onStatusChange?.(info),
  });
