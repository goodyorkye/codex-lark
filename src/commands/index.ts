...
async function handleTask(args: string, ctx: CommandContext): Promise<void> {
  const [action, threadId] = args.trim().split(/\s+/, 2);
  if (!action) {
    await handleTasks('', ctx);
    return;
  }
  ...
  if (!threadId || !ctx.agent.readThread) {
    await reply(ctx, '用法：`/task use <任务ID>`、`/task show <任务ID>` 或 `/task new`。');
    return;
  }
  ...
  if (action !== 'use') {
    await reply(ctx, '未知任务操作。');
    return;
  }
  if (!ctx.sessionCatalog) {
    await reply(ctx, '本地任务索引尚未初始化，请重启 codex-lark。');
    return;
  }
...
async function handleModels(_args: string, ctx: CommandContext): Promise<void> {
  if (!ctx.agent.listModels) {
    await reply(ctx, '当前 Codex 后端不支持模型列表。');
    return;
  }
...
async function handleApproval(args: string, ctx: CommandContext): Promise<void> {
  const [action, approvalId] = args.trim().split(/\s+/, 2);
  if (!approvalId || !ctx.agent.resolveApproval) {
    await reply(ctx, '审批请求无效或已经过期。');
    return;
  }
