...
type Group = ToolGroup | TextGroup | ApprovalGroup;
...
export function renderCard(state: RunState, options: RunCardRenderOptions = {}): object {
  ...
  for (const group of groupBlocks(state.blocks)) {
    if (group.kind === 'text') {
      ...
      elements.push(approvalPanel(group.approval, options));
    }
  }
...
function* groupBlocks(blocks: Block[]): Generator<Group> {
  ...
  for (const b of blocks) {
    if (b.kind === 'tool') {
      ...
      yield { kind: 'approval', approval: b.approval };
    }
  }
  if (toolBuf.length > 0) yield { kind: 'tools', tools: toolBuf };
}

function approvalPanel(approval: ApprovalEntry, options: RunCardRenderOptions): object {
  if (approval.status === 'resolved') {
    return noteMd(`✓ ${approval.title}已处理`);
  }
  const buttons: object[] = [
    approvalButton('允许一次', 'primary', `approval.accept ${approval.id}`, options),
  ];
  if (approval.allowForSession) {
    buttons.push(
      approvalButton('本次任务均允许', 'default', `approval.accept-session ${approval.id}`, options),
    );
  }
  buttons.push(approvalButton('拒绝', 'danger', `approval.decline ${approval.id}`, options));
  return {
    tag: 'column_set',
    flex_mode: 'none',
...
function approvalButton(
  ...
  const value: Record<string, unknown> = { cmd, arg };
  if (options.signCallback && cmd) {
    value.__bridge_cb = true;
    value.bridge_token = options.signCallback(cmd);
  }
  return {
    tag: 'button',
