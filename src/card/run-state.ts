...
export type Block =
  ...
  | { kind: 'approval'; approval: ApprovalEntry };
        ...
        blocks: [
          ...closeStreamingText(state.blocks),
          {
            ...
            approval: {
              ...
              title: evt.title,
              detail: evt.detail,
              allowForSession: evt.allowForSession,
              status: 'pending',
            },
          },
        ],
    ...
    case 'approval_resolved': {
      return {
        ...
        blocks: state.blocks.map((block) =>
          block.kind === 'approval' && block.approval.id === evt.approvalId
            ? { ...block, approval: { ...block.approval, status: 'resolved' } }
            : block,
        ),
        footer: 'thinking',
      };
