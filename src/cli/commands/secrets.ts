...
export async function runSecretsSet(
  ...
  if (!appId) {
    console.error('用法: codex-lark secrets set --app-id <id>');
    process.exit(1);
  }
  ...
  console.log(`✓ 已加密存到 ~/.codex-lark/profiles/<profile>/secrets.enc`);
}
...
export async function runSecretsRemove(
  ...
  if (!appId) {
    console.error('用法: codex-lark secrets remove --app-id <id>');
    process.exit(1);
  }
