...
export async function runStart(opts: StartOptions): Promise<void> {
  opts.onStatus?.('loading', '正在读取本机配置');
  ...
  opts.onStatus?.('checking', '正在检查 Codex Desktop 和飞书连接');
  ...
  for (;;) {
    try {
      ...
      await withProfileAndAppLocks(
        ...
        opts.onStatus?.('connecting', '正在建立飞书长连接');
...
export function assertReconnectAgentKindUnchanged(
  ...
  const currentKind = current ?? 'codex';
  const nextKind = next ?? 'codex';
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    throw new Error(
      `当前 ${err.kind === 'profile' ? 'profile' : 'app'} 已有 bridge 进程占用；` +
        '非交互模式无法确认停止，请先用 `codex-lark ps` 查看并用 `codex-lark kill <bot id>` 停止后重试',
    );
  }
