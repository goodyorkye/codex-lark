...
describe('profile runtime resolver', () => {
  ...
  it('does not treat PATH-only coding CLIs as a Desktop installation', async () => {
    const root = await tmpRoot();
    ...
    process.env.LARK_CHANNEL_CODEX_BIN = join(root, 'missing-desktop-core');
    ...
    try {
      ...
      expect(message).toContain('ChatGPT/Codex Desktop');
    } finally {
  ...
  it('continues first-run bootstrap with an explicit Desktop-core override', async () => {
    const root = await tmpRoot();
    const bin = join(root, 'bin');
  ...
  it('uses the requested agent when migrating a legacy config into an explicit profile', async () => {
    ...
    try {
      ...
      if (oldCodex === undefined) delete process.env.LARK_CHANNEL_CODEX_BIN;
      else process.env.LARK_CHANNEL_CODEX_BIN = oldCodex;
    }
  });
